...
if sys.version_info[0] < 3:
    def telet(): return os.system('pip install -U telethon')
elif sys.version_info[0] >= 3:
    def telet(): return os.system('pip3 install -U telethon')
...
async def main():
    ...
    async def getmem():
        ...
        target_group_entity = InputPeerChannel(
            channel[opt1].id, channel[opt1].access_hash)
        my_participants_id = []
            ...
            if user['uid'] in my_participants_id:
                ...
                try:
                    user_to_add = InputPeerUser(
                        user['uid'], user['access_hash'])
                    add = await client(InviteToChannelRequest(target_group_entity, [user_to_add]))
                    print(gr+'Added ', str(user['uid']))

                except PeerFloodError:
                    print(
                        re+"Getting Flood Error from telegram. Script is stopping now. Please try again after some time.")
                    i += 1
